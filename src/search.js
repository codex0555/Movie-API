...
search.get('/search/:l/:page', async ( req, res )=>{
    try {
        const l = req.params.l;
        var page = req.params.page || 1;
        const movieurl = `https://proxy.techzbots1.workers.dev/?u=https://123movies.org.mx/search/${l}/page/${page}`;
        ...
        $('.row.list-movie.g-3:first').find('.col').each(( index, element )=>{
            ...
            searchq.push({ name, type, img, id});
        })


        res.status(200).json({ pnex, searchq, searchk});
    } catch (error) {
